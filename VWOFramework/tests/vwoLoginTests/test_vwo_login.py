...
def test_vwo_login_negative(setup):
    try:
        ...
        loginPage.login_to_vwo(usr="py2x@thetestingacademy.com",pwd="admin")
        time.sleep(10)
...
def test_vwo_login_positive(setup):
    driver = setup
    loginPage = LoginPage(driver)
    loginPage.login_to_vwo(usr="mudlh2vt1h@ezztt.com",pwd="Wingify@123")
    time.sleep(10)
    dashboardPage = DashboardPage(driver)
    assert "Dashboard" in driver.title
    assert "Py2xATB" in dashboardPage.user_logged_in_text()
